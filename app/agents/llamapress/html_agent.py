...
async def view_current_page(
    ...
    try:
        ...
        error_message = "Error: Could not connect to server. Make sure your app is running."
        return Command(
            update={
                "messages": [ToolMessage(error_message, tool_call_id=tool_call_id)]
            }
...
async def edit_file(
    ...
    if not current_html:
        error_message = "Error: No current page HTML available to edit"
        return Command(
            update={
                "messages": [ToolMessage(error_message, tool_call_id=tool_call_id)]
            }
    ...
    try:
        ...
        if response.status_code == 200:
            success_message = f"Successfully edited the page (replaced 1 instance)"
            return Command(
                update={
                    "messages": [ToolMessage(success_message, tool_call_id=tool_call_id)]
                }
